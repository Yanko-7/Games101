...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
  ...
  for (const auto &t : TriangleList) {
    ...
    // mvp后的坐标，w非1，w == z 用于透视除法 且 in [-1, 1]
    ...
    for (int y = left_bot.y(); y <= right_top.y(); y++) {
      if (insideTriangle(x, y, v.data())) {
        ...
        float zp = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() +
                   gamma * v[2].z() / v[2].w(); // 二维屏幕空间对z进行插值
        zp *= Z;                                // 修正
        ...
        Vector2f interpolated_texcoords = t.tex_coords[0] * alpha +
                                          t.tex_coords[1] * beta +
                                          t.tex_coords[2] * gamma;
        Vector3f interpolated_shadingcoords =
            view_pos[0] * alpha + view_pos[1] * beta + view_pos[2] * gamma;
