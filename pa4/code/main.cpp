...
cv::Point2f recursive_bezier(const std::vector<cv::Point2f> &control_points,
                             float t) {
  int siz = control_points.size();
  auto tmp_points = control_points;
  for (int i = 1; i < siz; i++) {
    std::vector<cv::Point2f> tmp_new_points;
    for (int i = 1; i < tmp_points.size(); i++) {
      tmp_new_points.push_back(tmp_points[i - 1] +
                               (tmp_points[i] - tmp_points[i - 1]) * t);
    }
    swap(tmp_new_points, tmp_points);
  }
